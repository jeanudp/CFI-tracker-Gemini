...
import { BFR_GROUND_AREAS, BFR_FLIGHT_AREAS } from '../constants';
...
export default function FlightReview() {
  ...
  const totalGroundTasks = BFR_GROUND_AREAS.reduce((acc, area) => acc + area.tasks.length, 0);
  const totalFlightTasks = BFR_FLIGHT_AREAS.reduce((acc, area) => acc + area.tasks.length, 0);
  ...
  const handleClear = () => {
    if (!confirm('Clear all covered tasks and notes? This cannot be undone.')) return;
    setGroundCovered({});
    setFlightCovered({});
    setGroundNotes({});
    setFlightNotes({});
  ...
  const saveLesson = async (isGroundDraft: boolean) => {
    ...
    try {
      ...
      const bfr_endorsed = isGroundDraft ? false : (overallGrade === 'S');
      ...
      if (!isGroundDraft && overallGrade === 'S') {
        const { error: updateError } = await supabase
          .from('students')
          .update({ last_flight_review_date: lessonDate })
          .eq('name', studentName)
          .eq('user_id', session.user.id);
  ...
  if (loadingDraft) {
    return (
      <div className="min-h-screen bg-[#fafbfd] flex flex-col items-center justify-center p-8">
        ...
        <p className="text-sm font-medium text-[#6b7280]">Loading flight review draft...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-[#6b7280]">
            ...
            <span className="text-[#1a3a5c]">Flight Review (§61.56)</span>
          </div>
          <div className="flex items-center gap-4">
            <div className="w-10 h-10 bg-[#e8a020] rounded-lg flex items-center justify-center text-[#1a3a5c] text-xl font-bold">🛡️</div>
            <div>
              <h1 className="text-xl font-bold text-[#1c2333]">Flight Review (§61.56)</h1>
              <p className="text-xs text-[#6b7280]">§61.56 / AC 61-98E — Rating Independent</p>
            </div>
          </div>
        </div>
        <div className="flex gap-2">
          <Link to="/dashboard" className="flex items-center gap-2 px-4 py-2 rounded-lg border border-[#dde3ec] bg-white text-[#6b7280] hover:bg-[#f4f5f7] hover:-translate-y-0.5 transition-all text-xs font-bold">
            <ArrowLeft size={16} />
            Back
          </Link>
        </div>
      ...
      {/* Resume banner */}
      {lessonId && (
        <div id="resume-banner" className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-amber-800 text-xs sm:text-sm font-medium flex items-center gap-3 shadow-sm">
          <BookOpen className="text-amber-600 shrink-0" size={18} />
          <span>Resuming an in-progress flight review — complete the flight activities below and finalize.</span>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-[#dde3ec] shadow-sm p-6 mb-6">
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            placeholder="General flight review notes..."
            rows={2}
      ...
      <div className="bg-white rounded-2xl border border-[#dde3ec] shadow-md overflow-hidden mb-6">
        <button
          ...
          className="w-full bg-[#1a3a5c] text-white px-5 py-4 font-bold flex justify-between items-center hover:bg-[#1a3a5c]/95 transition-colors text-left"
        >
          <div className="flex items-center gap-3">
            <BookOpen size={20} className="text-[#e8a020]" />
          ...
          {groundExpanded && (
            ...
            >
              ...
              <div className="divide-y divide-[#dde3ec]">
                {BFR_GROUND_AREAS.map((area) => {
                  return (
                    <React.Fragment key={area.area}>
                      <div className="bg-[#1a3a5c] text-white px-4 py-2.5 text-[11px] font-bold flex justify-between items-center">
                            ...
                            <div className="p-4">
                              ...
                              {isExpanded && (
                                <motion.div
                                  initial={{ height: 0, opacity: 0 }}
                                  animate={{ height: 'auto', opacity: 1 }}
                                  className="mt-3 p-3 bg-[#f8fafc] rounded-lg border-l-4 border-[#1a3a5c] space-y-1.5 text-[11px]"
                                >
                                  <div>
                              ...
                              <button
                                ...
                                className={cn(
                                  ...
                                  isCovered
                                    ...
                                    : "bg-[#f4f5f7] border-[#dde3ec] text-[#94a3b8] hover:border-[#1a3a5c] bg-white"
                                )}
                              >
      ...
      <div className="bg-white rounded-2xl border border-[#dde3ec] shadow-sm p-6 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-xl mx-auto">
          ...
          >
            <span className="text-sm font-bold text-center">S — Satisfactory / Endorsement Given</span>
          </button>
          <button
          ...
          >
            <span className="text-sm font-bold text-center">N — Unsatisfactory / Dual Instruction Given</span>
          </button>
        </div>
        <p className="text-[11px] text-[#94a3b8] text-center mt-3 font-medium leading-relaxed">
          Per §61.56, an unsatisfactory review is logged as dual instruction given — no endorsement is issued.
        </p>
      </div>
