...
async function startServer() {
  ...
  app.get('/api/prog-chart', async (req, res) => {
    try {
      const { default: handler } = await import('./api/prog-chart.js');
      await handler(req, res);
    } catch (error) {
      console.error('Error in prog-chart handler:', error);
      res.status(500).json({ error: 'Internal Server Error' });
    }
  });
  ...
  app.get('/api/taf', async (req, res) => {
    try {
      const { default: handler } = await import('./api/taf.js');
      await handler(req, res);
    } catch (error) {
      console.error('Error in taf handler:', error);
      res.status(500).json({ error: 'Internal Server Error' });
    }
  });
